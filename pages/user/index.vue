      ...
      <div class="mt-[30px] px-[15px]">
        <table class="w-full">
          ...
          <tr>
            ...
            <td class="w-[300px]">Tên Người Dùng</td>
            <td class="w-[120px]">Email</td>
            <td class="w-[150px]">Số điện thoại</td>
            <td class="w-[120px]">Giới tính</td>
            <td class="w-[120px]">Địa chỉ</td>
          ...
          <tr
              ...
              v-for="(user, index) in displayProducts"
              :key="user"
          >
            <td class="leading-[19.36px] text-center">{{ user.id }}</td>
            ...
            <td class="leading-[19.36px] pl-5">{{ user.phone }}</td>
            <td class="leading-[19.36px] pl-5">{{ user.gender }}</td>
            <td class="leading-[19.36px] pl-5">{{ user.address }}</td>
            <td>
              <div class="flex gap-2.5 pl-[85px]">
                <NuxtLink :to="`/product/edit/${user.id}`">
...
onMounted(async () => {
  // const res = await productService.Allproduct(storeStore.id);
  // if (res.status) {
  //   if (!res.data.length) {
...
const totalPages = computed(() => {
  return Math.ceil(totalUser.value / itemsPerPage.value);
});
